...
describe('Register Service', () => {
	...
	it('should verify email', () => {
		...
		const req = httpMock.expectOne((req:any) => req.method === 'POST' && req.url === `${baseUrl}/verifyEmail?token=${token}`);
		req.flush(sampleRes);
	});
	...
	it('should register new user', () => {
		...
		const req = httpMock.expectOne((req:any) => req.method === 'POST' && req.url === url);
		req.flush(sampleRes);
	});
	...
	it('should propagate error response in case of client side error', () => {
		...
		httpMock.expectOne((req:any) => req.method === 'POST' && req.url === url).error(new ErrorEvent('network error'));
	});
});