...
public class UsersroleAppService implements IUsersroleAppService {
	...
	public UpdateUsersroleOutput update(UsersroleId usersroleId, UpdateUsersroleInput input) {
		...
		Usersrole existing = _usersroleRepository.findById(usersroleId).orElseThrow(() -> new EntityNotFoundException("Usersrole not found"));
	...
	protected BooleanBuilder searchKeyValuePair(QUsersrole usersrole, Map<String,SearchFields> map,Map<String,String> joinColumns) {
        ...
        while (iterator.hasNext()) {
			...
			if(details.getKey().replace("%20","").trim().equals("usersId")) {
				if(details.getValue().getOperator().equals("contains") && StringUtils.isNumeric(details.getValue().getSearchValue())) {
					...
					builder.and(usersrole.usersId.eq(Integer.valueOf(details.getValue().getSearchValue())));
				} else if(details.getValue().getOperator().equals("notEqual") && StringUtils.isNumeric(details.getValue().getSearchValue())) {
					builder.and(usersrole.usersId.ne(Integer.valueOf(details.getValue().getSearchValue())));
				} else if(details.getValue().getOperator().equals("range")) {
				   if(StringUtils.isNumeric(details.getValue().getStartingValue()) && StringUtils.isNumeric(details.getValue().getEndingValue())) {
                	   builder.and(usersrole.usersId.between(Integer.valueOf(details.getValue().getStartingValue()), Integer.valueOf(details.getValue().getEndingValue())));
                   } else if(StringUtils.isNumeric(details.getValue().getStartingValue())) {
                	   builder.and(usersrole.usersId.goe(Integer.valueOf(details.getValue().getStartingValue())));
                   } else if(StringUtils.isNumeric(details.getValue().getEndingValue())) {
                	   builder.and(usersrole.usersId.loe(Integer.valueOf(details.getValue().getEndingValue())));
				   }
				}
			}
		...
		if(joinCol != null && joinCol.getKey().equals("usersId")) {
		    builder.and(usersrole.users.id.eq(Integer.parseInt(joinCol.getValue())));
		}
	...
	public UsersroleId parseUsersroleKey(String keysString) {
		...
		usersroleId.setUsersId(Integer.valueOf(keyMap.get("usersId")));
		return usersroleId;
