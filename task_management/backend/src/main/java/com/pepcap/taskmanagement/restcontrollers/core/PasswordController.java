...
import com.pepcap.taskmanagement.application.core.authorization.tokenverification.ITokenVerificationAppService;
...
import com.pepcap.taskmanagement.addons.email.application.mail.IEmailService;
...
public class PasswordController {
	...
	public ResponseEntity<HashMap<String,String>> processForgotPassword(@RequestBody ForgotPasswordInput passwordInput, HttpServletRequest request) throws InvalidInputException {
		...
		FindUsersByUsernameOutput foundUsers = _usersAppService.findByEmail(passwordInput.getEmail());
	    if(foundUsers == null) {
	    	throw new EntityNotFoundException(String.format("There does not exist a users with a email=%s", passwordInput.getEmail()));
	    }
