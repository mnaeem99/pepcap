...
export class BaseListComponent<E> implements OnInit {
      ...
      .pipe(
        ...
        switchMap(() => {
          this.isLoadingResults = true;
            ...
            let result:any=[];
            if (this.selectedAssociation !== undefined && this.selectedAssociation.column && this.selectedAssociation.column.length>0) {
              ...
              result=this.dataService.getAll(this.searchValue, this.currentPage * this.pageSize, this.pageSize, sortVal);
            }
            return result as [];
          }),
        map((data) => {
    ...
    this.associations.forEach((association: any, associationIndex: any) => {
      ...
      if (matchedColumns == totalCount) {
        ...
        if(this.selectedAssociation.column){
          this.selectedAssociation.service?.getById(ServiceUtils.encodeId(this.selectedAssociation.column))
          .subscribe((parentObj) => {
  ...
  openDetails(item: E) {
    this.router.navigate([
      `${this.url}/${ServiceUtils.encodeIdByObject(item, this.primaryKeys)}`,
    ]);
  }
