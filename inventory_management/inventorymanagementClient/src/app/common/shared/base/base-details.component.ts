...
export class BaseDetailsComponent<E> implements OnInit, CanDeactivateGuard {
  ...
  onSubmit(data: any, headers?: any) {
    ...
    this.loading = true;
    this.dataService
      .update(data, this.idParam, headers)
      .pipe(first())
      .subscribe(
        (data) => {
          this.loading = false;
...
association.service?.getAll(this.searchValuePicker, this.currentPickerPage * this.pickerPageSize, this.pickerPageSize)      .subscribe(
  ...
  onPickerClose(associatedItem: any, association: IAssociationEntry) {
    ...
    this.itemForm.get(association.descriptiveField)?.setValue(association.referencedDescriptiveField?associatedItem[association.referencedDescriptiveField]:null);    }
  }
