...
public class ProductsControllerTest extends DatabaseContainerConfig{
	...
	public void setup() {
		...
		final ProductsController productsController = new ProductsController(productsAppService, categoriesAppService, inventoryTransactionsAppService,
		logHelper,env);
	...
	@Test
	public void GetInventoryTransactions_searchIsNotEmptyAndPropertyIsValid_ReturnList() throws Exception {

		Map<String,String> joinCol = new HashMap<String,String>();
		joinCol.put("id", "1");
		
        Mockito.when(productsAppService.parseInventoryTransactionsJoinColumn("productId")).thenReturn(joinCol);
		mvc.perform(get("/products/1/inventoryTransactions?search=productId[equals]=1&limit=10&offset=1")
				.contentType(MediaType.APPLICATION_JSON))
	    		  .andExpect(status().isOk());
	}  
	
	@Test
	public void GetInventoryTransactions_searchIsNotEmpty() {
	
		Mockito.when(productsAppService.parseInventoryTransactionsJoinColumn(anyString())).thenReturn(null);
	 		  		    		  
	    org.assertj.core.api.Assertions.assertThatThrownBy(() -> mvc.perform(get("/products/1/inventoryTransactions?search=productId[equals]=1&limit=10&offset=1")
				.contentType(MediaType.APPLICATION_JSON))
