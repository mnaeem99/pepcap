package com.pepcap.inventorymanagement.application.core.products;
...
import com.pepcap.inventorymanagement.domain.core.products.QProducts;
...
public class ProductsAppService implements IProductsAppService {
	...
	public UpdateProductsOutput update(Integer productsId, UpdateProductsInput input) {
		...
		products.setInventoryTransactionsSet(existing.getInventoryTransactionsSet());
		Categories foundCategories = null;
        ...
        while (iterator.hasNext()) {
			...
			if(details.getKey().replace("%20","").trim().equals("updatedAt")) {
		     ...
		     if(details.getKey().replace("%20","").trim().equals("categories")) {
				if(details.getValue().getOperator().equals("contains") && StringUtils.isNumeric(details.getValue().getSearchValue())) {
					builder.and(products.categories.id.like(details.getValue().getSearchValue() + "%"));
				} else if(details.getValue().getOperator().equals("equals") && StringUtils.isNumeric(details.getValue().getSearchValue())) {
					builder.and(products.categories.id.eq(Integer.valueOf(details.getValue().getSearchValue())));
				} else if(details.getValue().getOperator().equals("notEqual") && StringUtils.isNumeric(details.getValue().getSearchValue())) {
					builder.and(products.categories.id.ne(Integer.valueOf(details.getValue().getSearchValue())));
				} else if(details.getValue().getOperator().equals("range")) {
				   if(StringUtils.isNumeric(details.getValue().getStartingValue()) && StringUtils.isNumeric(details.getValue().getEndingValue())) {
                	   builder.and(products.categories.id.between(Integer.valueOf(details.getValue().getStartingValue()), Integer.valueOf(details.getValue().getEndingValue())));
                   } else if(StringUtils.isNumeric(details.getValue().getStartingValue())) {
                	   builder.and(products.categories.id.goe(Integer.valueOf(details.getValue().getStartingValue())));
                   } else if(StringUtils.isNumeric(details.getValue().getEndingValue())) {
                	   builder.and(products.categories.id.loe(Integer.valueOf(details.getValue().getEndingValue())));
				   }
				}
			}
		}
	...
	public Map<String,String> parseInventoryTransactionsJoinColumn(String keysString) {
